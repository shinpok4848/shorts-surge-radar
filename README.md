...
### 시장 레이더 Top 200
- YouTube 검색 결과를 50개씩 최대 4페이지 조회해 중복 제거 후 최대 200개 후보 수집
- 선택 국가의 언어와 60초 이하 조건을 엄격 적용하고, 해외 영상으로 되돌리는 fallback은 사용하지 않음
- 전체 결과는 자동 기획에 사용하고 화면은 20개씩 페이지로 표시
- 시장→런치 시 반복 키워드, 서로 다른 채널 수, 확산 점수, 반응률, 순위를 합산해 강한 주제와 신뢰도 자동 선정
- 숫자·목록형, 실수→해결형, 전후·변화형, 질문·이유형, 반전·대조형 제목 구조를 근거 영상과 함께 추론
- 시장 초기화는 시장·런치·제작 연결 상태를 비우고, 런치 초기화는 시장 결과를 유지한 채 기획만 다시 시작
