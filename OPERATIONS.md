...
- 시장 Top 200 스캔은 `search.list`를 50개씩 최대 4페이지 요청하고, 상세정보는 `videos.list`로 50개씩 배치 조회합니다.
- 전체 200개는 주제·구조 자동 기획에 사용하고 화면은 20개씩 페이지로 표시합니다.
- 비용이 큰 검색은 사용자가 명시적으로 시장 스캔 또는 연관영상 비교를 누를 때만 실행합니다.
